...
SHIM = r'''-- ============================================================
-- PICO-8 compatibility shim (generated by p8_lua_port.py)
...
do
  ...
  function music(n)
    ...
    -- song starts to moy track ids, nearest-lower for a mid-song index.
  ...
  -- moy lifecycle -> the p8 one, paced at PICO-8's fixed 30fps
  local m_clip = clip
...
def port(p8_path, out_dir, title=None):
    ...
    header = ("-- %s -- ported from PICO-8 by p8_lua_port.py.\n"
